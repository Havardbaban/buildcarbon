...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    const interestSavings =
      ((inputs.rateCurrent - inputs.rateGreen) / 100) * inputs.loanAmount;
    const taxSavings = (inputs.capex * inputs.taxRate) / 100;
    const recurringAnnual =
      energySavings + interestSavings + inputs.consultantFees;
    const firstYearTotal = recurringAnnual + inputs.grant + taxSavings;
    const paybackYears =
      recurringAnnual > 0 ? inputs.capex / recurringAnnual : Infinity;
    return {
      energySavings,
      interestSavings,
      taxSavings,
  ...
  /* PDF DOWNLOAD (exports the #report-section) */
    ...
    <div className="min-h-screen bg-white text-gray-900">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 items-center">
          <div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight">
              ...
              <span className="block text-emerald-600">
                CO₂ & ESG reporting for construction SMEs
              </span>
            </h1>
            <p className="mt-4 text-lg text-gray-700">
              Upload your energy/fuel data and generate tender-ready reports —
              while uncovering real savings from energy, grants, taxes and green
              finance.
            </p>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field
                label="Annual energy/fuel spend (NOK)"
                name="energySpend"
                value={inputs.energySpend}
                onChange={onChange}
              />
              <Field
              ...
              />
              <Field
                label="One-off grant (NOK)"
                name="grant"
                value={inputs.grant}
                onChange={onChange}
              />
              <Field
                label="Outstanding loan (NOK)"
                name="loanAmount"
              ...
              <Field
                ...
                name="rateCurrent"
                value={inputs.rateCurrent}
                onChange={onChange}
              />
              <Field
                label="Green interest (%)"
                name="rateGreen"
                value={inputs.rateGreen}
                onChange={onChange}
              />
              <Field
                label="Green capex (NOK)"
                name="capex"
                value={inputs.capex}
                onChange={onChange}
              />
              ...
              />
              <Field
                label="Consultant fees replaced (NOK/yr)"
                name="consultantFees"
                value={inputs.consultantFees}
                onChange={onChange}
              />
            </div>

            {/* Results section (this is what gets exported) */}
            <div id="report-section" className="mt-5 p-4 bg-emerald-50 rounded-xl">
              <h3 className="font-semibold mb-2">Results</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                <KPI
                  label="Energy savings"
                  value={currency(results.energySavings)}
                />
                <KPI
                  label="Interest savings"
                  value={currency(results.interestSavings)}
                />
                <KPI
                  label="Tax savings (year 1)"
                  value={currency(results.taxSavings)}
                />
                <KPI
                  label="Recurring annual savings"
                  value={currency(results.recurringAnnual)}
                />
                <KPI
                  label="First-year total benefit"
                  value={currency(results.firstYearTotal)}
                />
                <KPI
              ...
              <p className="mt-3 text-xs text-gray-600">
                Illustrative ROI; actual results depend on measures and financing
                approval.
              </p>
            </div>
      ...
      <section className="px-6 py-10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6">
          <Feature
            title="Built for Norwegian tenders"
            desc="Generate simple CO₂ summaries aligned to local requirements and export project-ready PDFs."
          />
          <Feature
            title="Partner-friendly"
            desc="Consultants and suppliers can white-label reports and invite their clients."
          />
          <Feature
            title="No hardware required"
            desc="Start with spreadsheets and invoices; add integrations later (Tripletex, Visma, telematics)."
          />
        </div>
      </section>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-4xl mx-auto bg-gray-50 border rounded-2xl p-8 text-center">
          ...
          <p className="mt-2 text-gray-700">
            Bring one month of energy & fuel data. We’ll produce a free baseline
            report and savings plan.
          </p>
      ...
      <footer className="px-6 py-10 border-t text-sm text-gray-600">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
          <p>
            © {new Date().getFullYear()} BuildCarbon — Simple ESG for construction
            SMEs.
          </p>
